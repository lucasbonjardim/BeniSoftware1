...
uses
  ...
  System.Classes, Winapi.Windows, Vcl.Forms, System.IniFiles;
...
begin
  Tot1 := 0;
  Tot2 := 0;
  CodRef := Poem_Zeros(Codigo, Numero - 1); //strPadZeroL(Codigo, Numero - 1);
...
begin
   ...
   for f := 1 to length(s) do begin
      if pos(s[f],'0123456789.-') = 0 then begin
         s := copy(s,1,f-1);
         break;
      end;
   end;
